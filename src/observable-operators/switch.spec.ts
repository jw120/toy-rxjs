...
describe('switch operator (synchronous - which is the same as concatAll)', () => {
  ...
  itObs('should work with an error in first inner observable',
    ToyRx.Observable.from(toy2).switch(),
    RefRx.Observable.from(ref2).switch(),
    errorEmits('ee')
  );
  ...
  itObs('should work with an error in second inner observable stream',
    ToyRx.Observable.from(toy3).switch(),
    RefRx.Observable.from(ref3).switch(),
    errorEmits('ee', 1, 2)
  );
  ...
  itObs('should work with an error in the outer observable',
    ToyRx.Observable.from(toy4).switch(),
    RefRx.Observable.from(ref4).switch(),
    errorEmits('outer e')
  );
...
describeObsTimedAsync('switch operator (sync of async)',  'should work',
  () => toy1.switch(),
  () => ref1.switch(),
  [0, 0, 0, 0],
  completeEmits(5, 7, 9)
