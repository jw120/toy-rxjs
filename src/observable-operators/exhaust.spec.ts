...
describe('exhaust operator (synchronous - which is the same as concatAll)', () => {
  ...
  itObs('should work with an error in first inner observable',
    ToyRx.Observable.from(toy2).exhaust(),
    RefRx.Observable.from(ref2).exhaust(),
    errorEmits('ee')
  );
  ...
  itObs('should work with an error in second inner observable stream',
    ToyRx.Observable.from(toy3).exhaust(),
    RefRx.Observable.from(ref3).exhaust(),
    errorEmits('ee', 1, 2)
  );
  ...
  itObs('should work with an error in the outer observable',
    ToyRx.Observable.from(toy4).exhaust(),
    RefRx.Observable.from(ref4).exhaust(),
    errorEmits('outer e')
  );
...
describeObsTimedAsync('exhaust operator (sync of async)',  'should work',
  () => toy1.exhaust(),
  () => ref1.exhaust(),
  [0, 0, 0, 0],
  completeEmits(1, 2, 3)
...
describeObsTimedAsync('exhaust operator (async of async)',  'should work',
  () => toy2.exhaust(),
  () => ref2.exhaust(),
  [0, 0, 0, 0],
  completeEmits(1, 2, 3)
