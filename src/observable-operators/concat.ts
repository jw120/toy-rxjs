...
export function concat<T>(first: SubscribeFn<T>, second: SubscribeFn<T>): Observable<T> {
  ...
  return new Observable<T>((o: Observer<T>): TearDownLogic => {
    ...
    // we run the first subscriptions
    //   on completion we run the first unsub, skip the completiona and trigger the second

    let firstComplete: boolean = false;
    let secondComplete: boolean = false;

    let firstO: Observer<T> =  {
      next: (x: T) => o.next(x),
      error: (e: Error) => o.error(e),
      complete: () => { firstComplete = true; }
    };
    let secondO: Observer<T> =  {
      next: (x: T) => o.next(x),
      error: (e: Error) => o.error(e),
      complete: () => { secondComplete = true; }
    };

    let patchedSecond: SubscribeFn<T> = (obs: Observer<T>) => {
      const tearDown: TearDownLogic = second(obs);
      if (secondComplete) {
        callTearDownLogic(tearDown);
        o.complete();
      }
    };

    let patchedFirst: SubscribeFn<T> = (obs: Observer<T>) => {
      const tearDown: TearDownLogic = first(obs);
      if (firstComplete) {
        callTearDownLogic(tearDown);
        patchedSecond(secondO);
      }
    };

    patchedFirst(firstO);

    // return subscribe((obs: Observer<T>): TearDownLogic => {
    //   let firstTearDown: TearDownLogic = first(obs);
    //   return () => {
    //     callTearDownLogic(firstTearDown);
    //     if (firstComplete) {
    //       console.log('Chaining to second');
    //       let secondSub: Subscription = subscribe(second, {
    //         next: (x: T) => o.next(x),
    //         error: (e: Error) => o.error(e),
    //         complete: () => { secondComplete = true; }
    //       });
    //       // let secondTearDown: TearDownLogic = second(obs);
    ...
    //   addCompleteHook<T>(o, secondState, firstSub));
