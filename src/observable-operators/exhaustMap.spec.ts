...
describe('exhaustMap operator (synchronous)', () => {

  const in1: Array<number> = [ 1, 3, 2];
  itObs('should work with complete observables',
    ToyRx.Observable.from(in1).exhaustMap((x: number) => ToyRx.Observable.from(letterMap(x))),
    RefRx.Observable.from(in1).exhaustMap((x: number) => RefRx.Observable.from(letterMap(x))),
    completeEmits('a', 'c', 'c', 'c', 'b', 'b')
  );

  itObs('should work with complete observables and a result selector',
    ToyRx.Observable.from(in1).exhaustMap((x: number) => ToyRx.Observable.from(letterMap(x)), sel),
    RefRx.Observable.from(in1).exhaustMap((x: number) => RefRx.Observable.from(letterMap(x)), sel),
...
describeObsAsync('exhaustMap operator (synchronous of asynchronous)', 'should work',
